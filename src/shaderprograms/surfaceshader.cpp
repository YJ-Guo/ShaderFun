...
void SurfaceShader::setupMemberVars()
{
    ...
    unifCamera     = context->glGetUniformLocation(prog, "u_CameraPos");
...
void SurfaceShader::setCameraPosition(const glm::vec3 &cameraPos)
{
    ...
    // Handle to the camera position on the GPU
    context->glUniform3fv(unifCamera,
                          // The number of elements that are to be modified
                          1,
       // A pointer to an array of count values that will be used to update the specified uniform variable
                          &cameraPos[0]);
    }
