...
uniform vec3 u_CameraPos;    // The camera's position in world space
...
void main()
{
    ...
    //fs_CameraPos = inverse(u_View) * vec4(0,0,0,1);
     fs_CameraPos = vec4(u_CameraPos, 1);         // Pass in the camera position as pre-calculated to avoid inverse
